<template>
    <CrudPage title="Trusted Servers">
        <template #add="p">
            <AddTrustedServer :clients="allClients" :servers="trustedServers" @cancel="p.cancel"
        ...
        <LoadingPanel :status="status" @load="loadTrustedServers" auto-load>
            <table class="table">
                ...
                <tbody>
                    <template v-for="trustedServer in filteredTrustedServers.slice(pageStart, pageEnd)"
                        ...
                        <tr :class="{ 'table-light': trustedServer.editing }">
                            ...
                            <td class="actions">
                                ...
                                <button class="btn btn-icon" @click="handleDelete(trustedServer)">
                        ...
                        <tr v-if="trustedServer.editing" class="edit-row table-light">
                            <td colspan="4">
                                <EditTrustedServer :server="trustedServer" :servers="trustedServers"
                                    :clients="allClients" @cancel="trustedServer.editing = false"
            ...
            <Paging :items="filteredTrustedServers" :page-size="pageSize" @update="handlePagingUpdate" />
        </LoadingPanel>
    </CrudPage>
</template>
...
const filteredTrustedServers = computed(() => trustedServers.value.filter((c) => !c.client || c.client?.name.toLocaleLowerCase().includes(filterText.value.toLocaleLowerCase())));
...
async function handleDelete(trustedServer: IGridTrustedServer) {
    const confirmDelete = confirm(`Are you sure you want to delete server ${trustedServer.client?.name}?`);
    if (confirmDelete) {
        try {
            status.value = AsyncStatus.BUSY;
